...
//
//   r and t keys to roll,
//   w and s keys to pitch,
//   a and d keys to yaw,
//   8 key to increase speed and the 0 key to decrease speed.
//   M  key toggles wireframe mode
//   Space generates a new landscape
//
...
void keyboard(unsigned char key, int, int) {
    ...
    switch(key) {
        ...
        case '0': theShip.setSpeed(theShip.getSpeed() - deltaSpeed); break;
        case 'm': wireframe = !wireframe; break;
        case ' ': newLandscape();
        case 't': theShip.roll(angle); break;
        case 'r': theShip.roll(-angle); break;
        case 'a': theShip.yaw(angle); break;
        case 'd': theShip.yaw(-angle); break;
...
void writeHelpToConsole() {
    std::cout << "r/t = roll left / right\n";
    std::cout << "w/f - pitch down / up\n";
    std::cout << "a/d - yaw left / right\n";
    std::cout << "8/0 - speed up / slow down\n";
    std::cout << "m - toggle wireframe mode\n";
    std::cout << "SPACE - generate a new landscape\n";
}
